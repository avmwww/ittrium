...
#define INVALIDATE_L1P()    { L1PICR = L1PICR | L1P_INVALIDATE; while (L1PICR != 0x00000000u){} }
#define SETMODE_L1P(mode)  (CSR = (CSR & (~PCC_MASK)) | (mode&PCC_MASK))

#define BEGIN_CRITICAL_SECTION	{ BOOL cpu_locked = cpu_is_locked(); cpu_lock();
#define END_CRITICAL_SECTION	if(!cpu_locked) cpu_unlock(); }

/*extern unsigned int   IER_save_1;
